...
public class AdminServiceImpl implements AdminService {
    ...
    public boolean login(Admin admin) {
        // 封装查询用户信息的条件，条件是用户名
        Admin conf = new Admin();
        conf.setLoginName(admin.getLoginName());
        Admin userInDb = adminMapper.selectOne(conf);
        if (userInDb == null) {
            return false;
        }

        // 判断客户端传入用户名密码是否与数据库中的数据相同
        String passwordInDb = userInDb.getPassword();//密文密码
        String password = admin.getPassword();//客户端传入的用户密码

        boolean checkResult = BCrypt.checkpw(password, passwordInDb);
        return checkResult;
    }
